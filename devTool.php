...
if ($_GET['tag']) {
    $connect->execute_query("UPDATE `files` SET `tag`=? WHERE `name`=?",[urldecode($_GET['tag']),$fname]);
        $connect->execute_query("INSERT INTO `logs` (`action`,`type`,`account`) VALUES(CONCAT('Set ',?,'s tag to ',?),1,?)",[$fname,urldecode($_GET['tag']),$_COOKIE['user']]);
    header("Location: uploadui.php");
}
if ($_GET['filenewname']) {
    ...
    $connect->execute_query("INSERT INTO `logs` (`action`,`type`,`account`) VALUES(CONCAT('Set ',?,'s name to ',?),1,?)",[$fname,urldecode($_GET['filenewname']),$_COOKIE['user']]);
    header("Location: uploadui.php");
}
if ($_GET['action']=="setGolden") {
   switch ($connect->execute_query("SELECT `goldenFile` FROM `files` WHERE `name`=?",[$fname])->fetch_row()[0]) {
    case 2:
        ...
        $connect->execute_query("INSERT INTO `logs` (`action`,`type`,`account`) VALUES(CONCAT('Set file ',?,' to normal'),1,?)",[$fname,$_COOKIE['user']]);
        break;
    case 1:
        $connect->execute_query("UPDATE `files` SET `goldenFile`=2 WHERE `name`=?",[$fname]);
        $connect->execute_query("INSERT INTO `logs` (`action`,`type`,`account`) VALUES(CONCAT('Set file ',?,' to diamond'),1,?)",[$fname,$_COOKIE['user']]);
        break;
    case 0:
        $connect->execute_query("UPDATE `files` SET `goldenFile`=1 WHERE `name`=?",[$fname]);
        $connect->execute_query("INSERT INTO `logs` (`action`,`type`,`account`) VALUES(CONCAT('Set file ',?,' to golden'),1,?)",[$fname,$_COOKIE['user']]);
        break;
   };
